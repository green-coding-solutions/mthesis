...
SRC_PHP="/tmp/php-mandelbrot.php"
BIN="/tmp/php-mandelbrot"

cp /tmp/repo/benchmarks/php/mandelbrot/main.php "$SRC_PHP"
...
if [ -n "$OPCACHE_SO" ]; then
  exec "$PHP_BIN" -dzend_extension="$OPCACHE_SO" -dopcache.enable_cli=1 -dopcache.jit_buffer_size=64M -n -d short_open_tag=1 $SHMOP_ARG $PCNTL_ARG "$SRC_PHP" "\$@"
else
  exec "$PHP_BIN" -n -d short_open_tag=1 $SHMOP_ARG $PCNTL_ARG "$SRC_PHP" "\$@"
fi
EOF
